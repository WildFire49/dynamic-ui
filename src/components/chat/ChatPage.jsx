...
import {
  ...
  getFormSchemaById,
} from "../dynamic-form/sampleFormSchemas";

// Dynamic imports - lazy load heavy components with loading states
const Sidebar = dynamic(() => import("../Sidebar"), {
  ssr: false,
  loading: () => <Box sx={{ width: 280 }} />,
});

const StellarThinking = dynamic(
  () => import("../mui/StellarThinking"),
  { ssr: false }
);

const ChatSkeleton = dynamic(() => import("../mui/ChatSkeleton"), {
  ssr: false,
});

const InputWithRecording = dynamic(
  () => import("../mui/InputWithRecording"),
  { ssr: false }
);

const UserMenu = dynamic(() => import("../auth/UserMenu"), {
  ssr: false,
  loading: () => <Box sx={{ width: 40, height: 40 }} />,
});

const ChatMessage = dynamic(
  () => import("../mui/ChatMessage").then((mod) => mod.default),
  { ssr: false }
);

const ConfirmationDialog = dynamic(
  () => import("../mui/ConfirmationDialog"),
  { ssr: false }
);

const PDFNotificationPopup = dynamic(
  () => import("../mui/PDFNotificationPopup"),
  { ssr: false }
);

const TypingIndicator = dynamic(
  () => import("../mui/TypingIndicator"),
  { ssr: false }
);
